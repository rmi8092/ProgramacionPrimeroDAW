package argumentosMain.listaDeLaCompraLourdes;
...
public class ListaDeLaCompra {
	...
	boolean annadir(Articulo articulo) {
		if (listaDeArticulos.contains(articulo))
			return false;
		return listaDeArticulos.add(articulo);
	}
	...
	StringBuffer generarListaDeLaCompra() {
		...
		while (iterator.hasNext()) {
			...
			if (articulo.bajoMinimos())
				stringBuffer.append("\n-(" + Integer.valueOf(articulo.getExistencias() - articulo.getMinimo()) + ")-" + articulo.getNombre());
		}
		return stringBuffer;				//Por que hace Integer.valueOf(.....?? No basta con hacer la resta de Minimo - Existencias?
	}
}
